defmodule Orchestrator.Repo.Migrations.CreateDeployments do
  ...
  def up do
    ...
    create table(:deployments, primary_key: false) do
      ...
      add :progress_deadline_seconds, :integer, default: 600  # 10 minutes
      ...
      # Metadata
      add :triggered_by, :string  # user_id, automation, ci_cd
      add :trigger_source, :string  # manual, github, gitlab, jenkins
    ...
    create table(:deployment_revisions, primary_key: false) do
      ...
      add :artifact_hash, :string
      add :config_snapshot, :map, default: %{}  # Full configuration at this revision
    ...
    create table(:deployment_replicas, primary_key: false) do
      add :id, :uuid, primary_key: true
      add :deployment_id, references(:deployments, type: :uuid, on_delete: :cascade), null: false
      add :revision_id, references(:deployment_revisions, type: :uuid, on_delete: :cascade), null: false
      add :replica_name, :string, null: false
      add :machine_id, :uuid  # Link to machines table if exists
      add :pod_ip, :string
      add :host_ip, :string
      add :node_name, :string

      # Status
      add :status, :string, null: false  # pending, running, succeeded, failed, unknown
    ...
    create table(:traffic_routes, primary_key: false) do
      ...
      add :old_version_weight, :integer, default: 100  # Percentage (0-100)
    ...
    create table(:deployment_events, primary_key: false) do
      add :id, :uuid, primary_key: true
      add :deployment_id, references(:deployments, type: :uuid, on_delete: :cascade), null: false
      add :event_type, :string, null: false  # phase_change, replica_created, health_check, traffic_shift
      add :event_severity, :string, null: false  # info, warning, error
      add :phase, :deployment_phase
      add :message, :text, null: false
      add :reason, :string
      add :component, :string  # controller, health_checker, traffic_manager
    ...
    create table(:canary_analysis_results, primary_key: false) do
      ...
      add :traffic_percentage, :integer  # Canary traffic percentage
      ...
      add :recommendation, :string  # promote, abort, continue
    ...
    create table(:deployment_hooks, primary_key: false) do
      add :id, :uuid, primary_key: true
      add :deployment_id, references(:deployments, type: :uuid, on_delete: :cascade), null: false
      add :hook_type, :string, null: false  # pre_deployment, post_deployment, pre_rollback, post_rollback
      ...
      # Execution tracking
      add :status, :string  # pending, running, succeeded, failed, skipped
      add :started_at, :utc_datetime
      add :completed_at, :utc_datetime
      add :duration_ms, :integer
