defmodule Orchestrator.Repo.Migrations.CreateSecurityCompliance do
  ...
  def up do
    ...
    create table(:security_scans, primary_key: false) do
      ...
      add :scan_type, :string, null: false  # vulnerability, dependency, secret, configuration, compliance
      add :target, :string, null: false  # machine_id, service_name, image_name, repository
      add :target_version, :string
      add :scanner, :string, null: false  # trivy, clair, grype, semgrep, bandit
    ...
    create table(:vulnerabilities, primary_key: false) do
      ...
      add :cve_id, :string  # CVE-2023-12345
      add :cwe_id, :string  # CWE-79
      ...
      add :exploit_maturity, :string  # unproven, poc, functional, high
    ...
    create unique_index(:vulnerabilities, [:scan_id, :cve_id, :package_name], name: :vulnerabilities_unique_finding)
    ...
    create table(:security_policies, primary_key: false) do
      ...
      add :description, :text
      add :policy_type, :string, null: false  # vulnerability, access_control, encryption, network, audit
      add :enabled, :boolean, default: true
      add :severity_threshold, :vulnerability_severity  # Block deployments if vulnerabilities >= threshold
      ...
      add :enforcement_mode, :string, default: "audit"  # audit, enforce
    ...
    create table(:compliance_requirements, primary_key: false) do
      add :id, :uuid, primary_key: true
      add :framework_id, references(:compliance_frameworks, type: :uuid, on_delete: :delete_all), null: false
      add :requirement_id, :string, null: false  # SOC2-CC6.1, HIPAA-164.312(a)(1)
      add :title, :string, null: false
      add :description, :text
      add :category, :string  # access_control, encryption, audit, incident_response
      add :status, :compliance_status, null: false, default: "pending_review"
      add :automated_check, :boolean, default: false
      add :check_query, :text  # SQL or code to verify compliance
    ...
    create table(:compliance_evidence, primary_key: false) do
      add :id, :uuid, primary_key: true
      add :requirement_id, references(:compliance_requirements, type: :uuid, on_delete: :delete_all), null: false
      add :evidence_type, :string, null: false  # document, screenshot, log, scan_result, configuration
      add :title, :string, null: false
      add :description, :text
      add :file_path, :string
      add :file_url, :string
    ...
    create table(:encryption_keys, primary_key: false) do
      ...
      add :key_type, :string, null: false  # data, transport, signing, backup
      add :algorithm, :encryption_algorithm, null: false
      add :key_size_bits, :integer
      add :key_hash, :string  # Hash of public key for verification
      add :status, :string, null: false, default: "active"  # active, rotating, rotated, revoked
      add :created_at_epoch, :bigint  # For precise rotation tracking
    ...
    create table(:security_audit_logs, primary_key: false) do
      add :id, :uuid, primary_key: true
      add :event_type, :string, null: false  # authentication, authorization, access, modification, deletion
      add :event_category, :string, null: false  # security, compliance, encryption, vulnerability
      add :severity, :string, null: false  # critical, high, medium, low, info
      add :actor, :string, null: false  # user_id, service_account, api_key
      add :actor_ip, :string
      add :action, :string, null: false
      add :resource_type, :string
      add :resource_id, :string
      add :status, :string, null: false  # success, failure, denied
      add :reason, :text
    ...
    create table(:secret_scan_results, primary_key: false) do
      add :id, :uuid, primary_key: true
      add :scan_id, references(:security_scans, type: :uuid, on_delete: :delete_all), null: false
      add :secret_type, :string, null: false  # api_key, password, token, certificate, private_key
      add :file_path, :string, null: false
      add :line_number, :integer
      add :matched_rule, :string
      add :secret_hash, :string  # Hash for deduplication
      add :entropy_score, :decimal, precision: 5, scale: 2
      add :confidence, :string  # high, medium, low
      add :status, :string, default: "open"  # open, verified, false_positive, remediated
    ...
    create table(:security_incidents, primary_key: false) do
      add :id, :uuid, primary_key: true
      add :incident_id, :string, null: false  # INC-2024-001
      add :title, :string, null: false
      add :description, :text
      add :severity, :vulnerability_severity, null: false
      add :status, :string, null: false, default: "open"  # open, investigating, contained, resolved, closed
      add :category, :string, null: false  # data_breach, unauthorized_access, malware, dos, insider_threat
      add :detected_at, :utc_datetime, null: false
      add :detected_by, :string
      add :detection_method, :string  # automated, manual, external_report
      ...
      add :mttr_seconds, :integer  # Mean Time To Resolve
  ...
  def down do
    ...
    execute "DROP TRIGGER IF EXISTS trigger_update_framework_compliance ON compliance_requirements"
