defmodule OrchestratorWeb.DebugChannel do
  ...
  def handle_in("resize", %{"rows" => rows, "cols" => cols}, socket) when socket.assigns.mode == "shell" do
    session = socket.assigns.session
  ...
  def handle_info(:heartbeat, socket) do
    ...
    if session && session_expired?(session) do
      Logger.warn("Debug session expired: #{session.id}")
  ...
  def handle_info(:push_metrics, socket) when socket.assigns.mode == "inspect" do
    ...
    case fetch_machine_metrics(machine) do
      ...
      {:error, reason} ->
        Logger.warn("Failed to fetch metrics: #{inspect(reason)}")
    end
    schedule_metric_push()
    {:noreply, socket}
  end
  ...
  def terminate(reason, socket) do
    Logger.info("Debug channel terminated: reason=#{inspect(reason)} machine=#{socket.assigns[:machine_id]}")
  ...
  defp subscribe_to_logs(machine_id, _filters) do
    :ok
  end
  defp passes_filters?(log_entry, filters) do
    level_match = if filters.level do
      log_level_value(log_entry.level) >= log_level_value(filters.level)
    else
      true
    end
    pattern_match = if filters.filter do
      Regex.match?(~r/
    else
      true
    end
  ...
  defp fetch_machine_threads(machine, params) do
  ...
  defp fetch_machine_network(machine, params) do
    connections = [
      %{protocol: "tcp", local_addr: "0.0.0.0", local_port: 8080,
        remote_addr: "0.0.0.0", remote_port: 0, state: "LISTEN"}
    ]
    {:ok, %{connections: connections}}
  end
  defp fetch_machine_fds(machine, params) do
    fds = [
  ...
  defp set_fsm_breakpoint(machine_id, state, condition) do
