defmodule Orchestrator.Manager do
  ...
  defp handle_remote_creation(%Machine{} = machine, state) do
    case RegionRouter.create_machine(machine.name, machine.region) do
      ...
      {:error, reason} ->
        Logger.error(
          "Failed creating remote machine for local ID #{machine.id}: #{inspect(reason)}"
        )
  ...
  defp reconcile_machine(%Machine{} = machine) do
    ...
    with {:ok, remote_state} <- get_remote_state(machine),
         ...
         :ok <- maybe_update_metrics(machine, remote_state),
         :ok <- maybe_suggest_migration(machine) do
      :ok
    else
      {:error, :remote_not_found} ->
        Logger.warning(
          "Remote machine not found for local ID #{machine.id}. Attempting self-heal (recreate)...",
      ...
      {:error, reason} ->
        Logger.error("Reconciliation error for machine #{machine.id}: #{inspect(reason)}",
          machine_id: machine.id
        )
  ...
  defp get_remote_state(%Machine{} = machine) do
    Logger.warning(
      "Cannot get remote state for machine #{machine.id}: missing 'remote_id' in metadata.",
      machine_id: machine.id
    )
  ...
  defp maybe_update_local_status(%Machine{} = machine, %{"remote_status" => remote_status}) do
    if machine.status != remote_status do
      Logger.info(
        "Status mismatch for machine #{machine.id}: local=#{machine.status}, remote=#{remote_status}. Updating local.",
        machine_id: machine.id
      )
      ...
      case Repo.update(changeset) do
        ...
        {:error, update_cs} ->
          Logger.error(
            "Failed to update local status for machine #{machine.id}: #{inspect(update_cs.errors)}",
            machine_id: machine.id
          )
  ...
  defp maybe_update_metrics(%Machine{} = machine, remote_state) do
    cpu = :rand.uniform() * 100
    memory_mb = 512 + :rand.uniform(512)
    latency_ms = 10.0 + :rand.uniform() * 50.0

    cpu = Map.get(remote_state, "cpu", cpu)
    memory_mb = Map.get(remote_state, "memory_mb", memory_mb)
    latency_ms = Map.get(remote_state, "latency_ms", latency_ms)
    ...
    changeset = Ecto.Changeset.change(machine, changes)
    ...
    case Repo.update(changeset) do
      ...
      {:error, update_cs} ->
        Logger.error(
          "Failed to update metrics for machine #{machine.id}: #{inspect(update_cs.errors)}",
          machine_id: machine.id
        )
  ...
  defp maybe_suggest_migration(%Machine{} = machine) do
    case Predictor.should_migrate?(machine.id) do
      {:migrate, reason} ->
        Logger.info("Predictor suggests migration for machine #{machine.id}: #{reason}",
          machine_id: machine.id
        )
        ...
        if get_in(machine.metadata, ["suggestion"]) != reason do
          ...
          case Repo.update(changeset) do
            ...
            {:error, update_cs} ->
              Logger.error(
                "Failed to update machine #{machine.id} metadata with suggestion: #{inspect(update_cs.errors)}",
                machine_id: machine.id
              )
      ...
      :ok ->
        if Map.has_key?(machine.metadata || %{}, "suggestion") do
          Logger.info("Clearing previous migration suggestion for machine #{machine.id}",
            machine_id: machine.id
          )
          ...
          case Repo.update(changeset) do
            ...
            {:error, update_cs} ->
              Logger.error(
                "Failed to clear suggestion for machine #{machine.id}: #{inspect(update_cs.errors)}",
                machine_id: machine.id
              )
    ...
    case RegionRouter.create_machine(machine.name, machine.region) do
      {:ok, %{"id" => new_remote_id, "status" => new_status}} ->
        ...
        case Repo.update(changeset) do
          ...
          {:error, update_cs} ->
            Logger.error(
              "Self-heal failed: Could not update local record #{machine.id} after recreate: #{inspect(update_cs.errors)}",
              machine_id: machine.id
            )
      ...
      {:error, reason} ->
        Logger.error(
          "Self-heal failed: Could not recreate remote machine for local ID #{machine.id}: #{inspect(reason)}",
          machine_id: machine.id
        )
